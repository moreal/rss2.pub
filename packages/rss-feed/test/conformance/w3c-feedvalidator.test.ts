...
import {
  ...
  listSelectedW3cRss20Paths,
  readW3cRss20FixtureBytes,
  W3C_FEEDVALIDATOR_COMMIT,
  W3C_FEEDVALIDATOR_ROOT,
} from "./w3c-feedvalidator-support.js";

const accepted = W3C_RSS20_CASES.filter(
  (testCase) => testCase.classification === "accept",
);
const rejected = W3C_RSS20_CASES.filter(
  (testCase) => testCase.classification === "reject",
);

const REPOSITORY_ROOT = join(W3C_FEEDVALIDATOR_ROOT, "..", "..");
const UPDATE_SCRIPT = join(REPOSITORY_ROOT, "scripts", "update-w3c-rss20-manifest.mjs");
const DIRTY_FIXTURE_PATH = "testcases/rss20/data-types-characterdata/amp-cdata.xml";
const DIRTY_FIXTURE = join(W3C_FEEDVALIDATOR_ROOT, DIRTY_FIXTURE_PATH);
const UNTRACKED_FIXTURE_PATH =
  "testcases/rss20/data-types-characterdata/rss2pub-dirty-fixture.xml";
...
describe("W3C Feed Validator RSS 2.0 corpus", () => {
  it("uses the initialized, licensed, pinned upstream snapshot", () => {
    ...
    expect(listSelectedW3cRss20Paths()).toHaveLength(326);
  });
  ...
  it("accounts for every selected fixture exactly once", () => {
    const selected = listSelectedW3cRss20Paths();
    expect(W3C_RSS20_CASES.map((testCase) => testCase.path)).toEqual(selected);
    expect(new Set(W3C_RSS20_CASES.map((testCase) => testCase.path)).size).toBe(326);

    for (const testCase of W3C_RSS20_CASES) {
      expect(
        createHash("sha256")
          .update(readW3cRss20FixtureBytes(testCase.path))
          .digest("hex"),
        testCase.path,
  ...
  it("classifies every upstream no-error document without exclusions", () => {
    const noError = W3C_RSS20_CASES.filter(
      (testCase) => testCase.upstreamExpectation === "!Error",
    );
    expect(noError).toHaveLength(3);
    expect(noError.every((testCase) => testCase.classification === "accept"
      || testCase.classification === "project")).toBe(true);
  });

  it.each(accepted)("accepts RSS 2.0 $feature $path", (testCase) => {
    expect(parseW3cRss20Case(testCase), testCase.path).toMatchObject({ ok: true });
  });

  it.each(rejected)("rejects RSS 2.0 $feature $path as $expectedError", (testCase) => {
    expect(parseW3cRss20Case(testCase), testCase.path).toMatchObject({
      ok: false,
  ...
  it("leaves an existing probe file untouched when its precondition fails", () => {
    const temporaryDirectory = mkdtempSync(join(tmpdir(), "rss2pub-w3c-rss20-probe-"));
