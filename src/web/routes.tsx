...
import type { FindFeedByHandle } from "../application/find-feed-by-handle.js";
...
export function createWebRoutes(deps: WebDeps): Hono {
  ...
  app.get("/registered/:handle", negotiateLocale, async (c) => {
    const result = await deps.findFeedByHandle.execute(c.req.param("handle"));
    if (!result.ok) return c.notFound();
    return c.html(
      <RegisterResultPage
        ctx={pageContext(c, deps)}
        outcome={{
          kind: c.req.query("created") === "1" ? "created" : "exists",
          feed: result.value,
        }}
      />,
    );
  ...
  app.post("/register", negotiateLocale, async (c) => {
    ...
    return c.redirect(
      `/registered/${encodeURIComponent(result.value.feed.handle)}?${params}`,
      303,
    );
  });
