...
- 정규화 알고리즘 (canonical feed URL 기준, 결정적):
  ...
  3. **22자 초과 시**: 앞 22자로 절단(말미 `_` 재정리) + `_` + SHA-256(canonical URL)
     기반 base36 해시 앞 7자 → 최종 길이는 최대 30자(22 + 1 + 7)
- 대소문자 무시 조회(마스토돈/미스키 동작)를 전제로 소문자만 발급, 충돌은 해시로 회피.
...
- 메인 액터 명령: **`register <url>`, `search <keyword>`** 두 가지로 시작.
  unregister는 제공하지 않음.
- 핸들 정규화: **22자 초과 시** 해시 suffix 적용(최종 최대 30자).
